...
const steps: Step[] = [
  {
    label: "Recibido",
    description: "Tu vehiculo fue ingresado al taller",
    status: "done",
  ...
  {
    label: "Diagnostico",
    description: "Inspeccion completa realizada",
    status: "done",
  ...
  {
    label: "En Reparacion",
    description: "El tecnico esta trabajando en tu auto",
    status: "active",
    icon: <Wrench className="h-4 w-4" />,
  },
  {
    label: "Listo para Retiro",
    description: "Te avisaremos cuando este listo",
    status: "pending",
...
function StepIndicator({ status, icon }: { status: StepStatus; icon: React.ReactNode }) {
  ...
  if (status === "active") {
    return (
      <div className="relative z-10 flex h-9 w-9 items-center justify-center">
        ...
        <div className="relative flex h-9 w-9 items-center justify-center rounded-full bg-primary text-primary-foreground shadow-lg shadow-primary/40">
          {icon}
        </div>
      </div>
    )
  ...
  return (
    <div className="relative z-10 flex h-9 w-9 items-center justify-center rounded-full border-2 border-border bg-card text-muted-foreground">
      {icon}
    </div>
  )
}
