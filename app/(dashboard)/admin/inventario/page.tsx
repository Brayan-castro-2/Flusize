...
export default function InventoryPage() {
    ...
    const isAdminRole = user?.role === 'superadmin' || user?.role === 'admin' || user?.role === 'flusize_admin' || user?.role === 'taller_admin' || user?.role === 'mecanico';

    if (!isAdminRole && !hasPermission('inventario.ver')) {
        return (
                ...
                <div className="flex flex-col md:flex-row flex-wrap items-stretch md:items-center gap-4">
                    <div className="relative w-full md:flex-1 md:min-w-[200px]">
                        ...
                        <input
                            ...
                            className="w-full rounded-xl border border-slate-200 bg-white py-2.5 pl-10 pr-4 text-sm text-slate-900 placeholder-slate-400 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20 transition-all font-medium"
                            style={{ color: '#1e293b' }}
                        />
                    </div>
