      ...
      <div className="max-w-5xl mx-auto px-4 text-center z-10 flex flex-col items-center pt-4 pb-4">
        ...
        >
          ...
          <p className="text-base md:text-2xl text-slate-300 mb-6 md:mb-12 max-w-4xl mx-auto leading-relaxed drop-shadow-lg font-medium">
            El ecosistema que conecta a los mejores servicios automotrices con sus clientes. <strong className="text-white">Transparencia</strong>, <strong className="text-white">historial</strong>, <strong className="text-white">control total</strong> y <strong className="text-white">fiabilidad</strong>.
          </p>
...
const AutoCarouselBento = ({ isInView }: { isInView: boolean }) => {
  ...
  const tabs = [
    ...
    },
    {
      ...
      items: [
        { icon: Banknote, text: 'Control de inventario y margen exacto' },
        { icon: FileSignature, text: 'Contratos automáticos con firma remota' },
        { icon: Wrench, text: 'Gestión de elevadores y tiempos' },
        { icon: Car, text: 'Recepción de autos, repuestos o ECUs' },
        { icon: MapPin, text: 'Captación de clientes por Mapa B2C' },
        { icon: MessageSquare, text: 'Tracking y aprobaciones por WhatsApp' },
      ],
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-5 mt-6">
                  {tab.items.map((item, i) => (
                    <div key={i} className="flex items-start gap-3">
                      <div className={`shrink-0 w-8 h-8 rounded-lg bg-${tab.accent}-500/10 text-${tab.accent}-400 flex items-center justify-center`}>
                        <item.icon className="w-4 h-4" />
                      </div>
                      <span className="text-sm font-medium text-slate-200 leading-snug">{item.text}</span>
                    </div>
                  ))}
                </div>
              ) : (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 items-center">
              ...
              >
                <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-slate-800 border border-slate-700 text-slate-300 mb-8 shadow-sm">
                  ...
                  <span className="text-sm font-bold tracking-wide">Para Talleres, Flotas y Rent-a-Cars</span>
                </div>
                ...
                <p className="text-lg md:text-xl text-slate-300 mb-10 leading-relaxed font-medium">
                  En la nueva era digital, la desconfianza detiene tus elevadores y cada minuto de espera es dinero que no vuelve. Rompe el ciclo y transforma la forma en que operas: digitaliza el 100% de tu negocio, blinda tu rentabilidad y atrae clientes en piloto automático.
                </p>
                
                <div className="space-y-6 mb-10 h-[400px] overflow-y-auto custom-scrollbar pr-4">
                  {[
                    { icon: CheckCircle2, title: "Aprobación Instantánea (Cero Tiempos Muertos)", desc: "El cliente ve la falla en su pantalla y autoriza el presupuesto con un toque directamente desde su WhatsApp. Tus elevadores siempre en marcha. Si fluye la información, fluye el trabajo y entra el dinero." },
                    { icon: Banknote, title: "Rentabilidad Milimétrica", desc: "Deja de adivinar tus ganancias. El sistema cruza automáticamente los repuestos de tu inventario con cada orden y vehículo. Descubre tu margen de ganancia exacto al centavo y qué servicios te dejan más dinero." },
                    { icon: FileSignature, title: "Cero Papeleo y Ventas Blindadas", desc: "Lleva tus compraventas y arriendos al siglo XXI. Genera contratos automáticos y permite que tus clientes firmen con validez digital desde su propio celular. Cierra negocios a distancia sin imprimir una sola hoja." },
                    { icon: MapPin, title: "Nuevos Clientes en Piloto Automático", desc: "No eres un taller aislado, eres parte del ecosistema automotriz más moderno. Al unirte a Flusize, apareces en nuestro Mapa interactivo de Conductores y Red de Asistencia VIP. Te conectamos con usuarios que buscan servicios exactos en tu zona." },
                    { icon: Activity, title: "El Control de un CEO", desc: "Mide el rendimiento en tiempo real de tus mecánicos, controla el calendario de tu flota de rent-a-car y lleva el historial inmutable de cada cliente para fidelizarlos para siempre. Todo en un dashboard gerencial diseñado para ser fácil." },
                    { icon: Wrench, title: "Adaptabilidad Extrema", desc: "¿Reparas piezas sueltas como ECUs? ¿Vendes repuestos a otras regiones por mostrador? Nuestro sistema inteligente se adapta a ventas directas sin vehículo para que tu caja siga cuadrando a la perfección." }
                  ].map((item, i) => (
                    <div key={i} className="flex gap-4 group">
                      <div className="flex-shrink-0 w-12 h-12 rounded-xl bg-blue-500/10 border border-blue-500/20 flex items-center justify-center text-blue-400 shadow-sm group-hover:scale-110 group-hover:bg-blue-500/20 transition-all">
                ...
                <button 
                  ...
                  className="w-full md:w-auto px-8 py-4 bg-cyan-600 hover:bg-cyan-500 text-white rounded-2xl font-black shadow-[0_0_30px_rgba(8,145,178,0.4)] transition-all flex items-center justify-center gap-3 text-lg hover:scale-105 active:scale-95 border border-cyan-400/50 uppercase tracking-tight"
                >
                  Ver planes y potenciar mi negocio <ArrowRight className="w-5 h-5" />
                </button>
              </motion.div>
